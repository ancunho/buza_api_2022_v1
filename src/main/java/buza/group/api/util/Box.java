...
public class Box extends Hashtable {
    ...
    public void copyToEntity(Object entity) {
        ...
        for (int i = 0 ; i < field.size() ; i++) {
            try {
                ...
                if ( containsKey( fieldname ) ) {
                    if ( fieldtype.equals("java.lang.String")) {
                        field.get(i).set(entity, getString(fieldname));
                    }
                    else if ( fieldtype.equals("int")) {
                        field.get(i).setInt(entity, getInt(fieldname));
                    }
                    else if ( fieldtype.equals("double")) {
                        field.get(i).setDouble(entity, getDouble(fieldname));
                    }
                    else if ( fieldtype.equals("long")) {
                        field.get(i).setLong(entity, getLong(fieldname));
                    }
                    else if ( fieldtype.equals("float")) {
                        field.get(i).set(entity, new Float(getDouble(fieldname)));
                    }
                    else if ( fieldtype.equals("boolean")) {
                        field.get(i).setBoolean(entity, getBoolean(fieldname));
                    }
                }
            }
            catch(Exception e){
            ...
            field.get(i).setAccessible(false);
        }
    }

    public void copyToEntityMap(Map<String, Object> map) {
        if ( map == null )
            throw new NullPointerException("trying to copy from box to null map");
